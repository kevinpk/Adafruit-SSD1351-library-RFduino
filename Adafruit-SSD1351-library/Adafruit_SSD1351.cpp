...
//        *sclkport &= ~sclkpinmask;
        ...
        if (c & _BV(i)) {
            NRF_GPIO->OUTSET = (1 << _sid);
//            *sidport |= sidpinmask;
            //digitalWrite(_sid, HIGH);
            //SID_PORT |= _BV(SID);
        } else {
            NRF_GPIO->OUTCLR = (1 << _sid);
//            *sidport &= ~sidpinmask;
...
void Adafruit_SSD1351::writeCommand(uint8_t c) {
    NRF_GPIO->OUTCLR = (1 << _rs);
//    *rsport &= ~ rspinmask;
    //digitalWrite(_rs, LOW);
    
    NRF_GPIO->OUTCLR = (1 << _cs);
//    *csport &= ~ cspinmask;
...
//    digitalWrite(_cs, LOW);
    ...
    if (_rst) {
        pinMode(_rst, OUTPUT);
        NRF_GPIO->OUTSET = (1 << _rst);
//        digitalWrite(_rst, HIGH);
        delay(500);
        NRF_GPIO->OUTCLR = (1 << _rst);
